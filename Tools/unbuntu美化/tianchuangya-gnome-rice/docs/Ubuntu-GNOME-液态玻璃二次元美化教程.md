...
extension.js     # 悬浮球、拖动判断、换壁纸和剪贴板按钮
stylesheet.css   # 圆形缩略图、玻璃外圈和悬浮按钮样式
```
...
6. 只走 `button-press-event` / `button-release-event` 这一条事件路径，不再同时绑定 `clicked`，避免 GNOME Shell 偶发把点击状态卡住。
7. `_pull()` 使用 `try/finally` 释放 busy 锁，哪怕换壁纸过程中出现异常，下一次点击也不会永久失效。
...
之前尝试过窗帘、截图卡片和 Coverflow 3D 旋转，但在当前 GNOME Shell 环境下容易出现卡顿、空白帧或点击失效。最终保留直接切换，优先保证：
...
壁纸切换：58px 可拖动悬浮球、圆形当前壁纸缩略图、直接随机切换
```
